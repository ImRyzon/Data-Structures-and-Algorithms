...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        int n = r.readInt(), max = r.readInt(), w[] = new int[n];
        long v[] = new long[n], dp[] = new long[max+1];
        for (int i = 0; i < n; i++) {w[i] = r.readInt(); v[i] = r.readLong();}
        for (int i = 1; i <= n; i++) {
            for (int j = max; j >= 0; j--) {if (w[i-1] <= j) dp[j] = Math.max(dp[j], dp[(int)(j-w[i-1])] + v[i-1]);}
        }
        pr.println(dp[max]); pr.close();
    }
